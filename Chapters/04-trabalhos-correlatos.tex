...
Nas seções seguintes são apresentadas algumas iniciativas e esforços de
desenvolvimento que se relacionam com a proposta deste trabalho.
...
\section{Informações sobre pacotes}
...
\begin{sidewaystable}
  \centering
  \caption{Gerência de informações sobre pacotes Debian}
  \label{tab:recDebian}
  \small
...
\section{Trabalhos descontinuados}

Ao longo do desenvolvimento deste trabalho, esforços anteriores de
desenvolvimento na área de recomendação e mineração de dados sobre pacotes
Debian foram identificados e estão descritos abaixo.

%FIXME [questionamento aos desenvolvedores sobre razao da descontinuidade]

\subsection{PopSuggest/Anapop}

O \textit{PopSuggest}\footnote{http://www.enricozini.org/2007/debtags/popcon-play/}
oferecia recomendações a partir de dados do \textit{Popcon} como uma ilustração
das possibilidades de uso dos dados coletados.

\subsection{Debommender}

O \textit{Debommender}\footnote{http://ostatic.com/debommender} foi desenvolvido
como prova de conceito no âmbito de um trabalho de graduação, não sendo porém
integrado aos serviços da distribuição.

\subsection{Mineração de dados do Popcon}
